...
int sysinfo(int n) {
  ...
  if (n == 0) {
    ...
    //return the total number of system calls that has made so far since the system boot up.
    // -1 to avoid counting the current sysinfo call.
    return numSysCalls;
  }
  else if (n == 2) {
    //return the number of free memory pages in the system.
...
// int syscall_count; // the total number of system calls that the current process has made so far. Do not include the current sysinfo syscalls attempt when returning the number.
// int page_usage; // the current processs memory size in pages (e.g., 10000 bytes -> 3 pages).
// };
//
  struct proc* p = myproc(); // Call the current process
  struct pinfo param;
  param.ppid = parentID - 1; // using parentID defined in allocproc()
  param.page_usage = (PGROUNDUP(p->sz))/PGSIZE; //PGROUNDUP(size) will round up the size and PGSIZE is the page size
  param.syscall_count = p->syscall_count; // syscall_count defined in syscall
  int addr;
  argaddr(0, &addr);
  // Write it directly
  if(copyout(p->pagetable, addr, (char *)&param, sizeof(param)) < 0)
